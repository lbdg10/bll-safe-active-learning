...
y_min_C = 60
y_max_C = 100
y_min = (y_min_C - output_scaler_bias) / output_scaler_scale
y_max = (y_max_C - output_scaler_bias) / output_scaler_scale
...
# BNN prior
lambda_param = 0.5
theta_star = np.concatenate((Uo.flatten(), bo.flatten()), axis=0).reshape(-1, 1)
theta_0 = theta_star*0.2
Lambda_0 = np.eye(n_theta)*lambda_param
...
def mpc_exploration_pessimistic(x_n, Lambda_inv_n, Q_n, theta_mean_n, theta_std_n, y_pred_n, y_std_n, U_prev, X_prev, n):
    ...
    # Minimize
    opti.minimize(100 * ca.sum(slack_sigma) + J_step) #100
...
def get_informative_horizon(slack_seq, tol):
    ...
    H = slack_seq.shape[1]
    for h in range(H):
        if np.allclose(slack_seq[:, h], 0.0, atol=tol):
            return h
    return H-1 
...
while n < N:
    ...
    # Compute beta dynamically
    Lambda_k = torch.inverse(Lambda_inv)
    Lambda_k_np = Lambda_k.detach().cpu().numpy()
    Lambda_0_np = np.eye(n_theta)*lambda_param
    det_ratio = np.sqrt(np.linalg.det(Lambda_k_np) / np.linalg.det(Lambda_0_np))
    beta_n_comp = np.sqrt(2 * np.log((1 / delta_prob) * det_ratio)) + np.sqrt(C / sigma2)
    beta_vec[n] = beta_n_comp
    ...
    X_prev[:, :-1] = X_prev[:, 1:] 
    X_prev[:, -1] = X_prev[:, -1] 
    slack_matrix[n,:] = slack_vector.flatten()
    all_slack_nonzero = np.all(np.abs(slack_vector) > slack_tol)

    if not switched and all_slack_nonzero:
        switched = True

    if switched and not all_slack_nonzero:
        switched = False
          
    if not switched:

        # Find horizon until informative input
        h_star = get_informative_horizon(slack_vector, tol=slack_tol)
    ...
    t_total[n-1] = t_pess[n-1] + t_expl[n-1] 
...
pyplt.plot(time, (y_min_C)*np.ones((N,1)), color='black', linewidth=4)
pyplt.plot(time, (y_max_C)*np.ones((N,1)), color='black', linewidth=4)
pyplt.ylabel(r'Temperature [°C]', fontsize=44)
pyplt.xlabel(r'Time [h]', fontsize=44)
pyplt.ylim((y_min_C-1,y_max_C+1))
pyplt.xlim((0, N))
pyplt.xticks(ticks=hour_ticks, labels=hour_labels)
pyplt.tick_params(axis='both', labelsize=44)  
pyplt.subplots_adjust(bottom=0.18)
...
# ========== BETA ==========
pyplt.figure()
pyplt.plot(time, beta_vec, linewidth=4)
pyplt.xlabel(r'Time [h]', fontsize=44)
pyplt.ylabel(r'$\beta_{\delta,k}$', fontsize=44)
pyplt.tick_params(axis='both', labelsize=44)  
pyplt.subplots_adjust(bottom=0.18)
pyplt.show()
...
current_state = switched_logic[0]
start_idx = 0
for i in range(1, N):
    if switched_logic[i] != current_state:
        # Shade the region where the state was constant
        pyplt.axvspan(start_idx, i, 
                   facecolor='lightblue' if current_state == 0 else 'lightcoral',
                   alpha=0.3)
        start_idx = i
        current_state = switched_logic[i]
# Shade the final region
pyplt.axvspan(start_idx, N, 
           facecolor='lightblue' if current_state == 0 else 'lightcoral',
           alpha=0.3)
# --- LABELS AND AXES ---
...
for i in range(5):
    ...
    pyplt.ylabel(r'$w_' + str(i) + '$', fontsize=44)
    pyplt.xlabel(r'Time [h]', fontsize=44)
    pyplt.xlim((0, N))
    pyplt.xticks(ticks=hour_ticks, labels=hour_labels)
    pyplt.tick_params(axis='both', labelsize=44)  
...
pyplt.ylabel(r'$\text{Bias}$', fontsize=44)
pyplt.xlabel(r'Time [h]', fontsize=44)
pyplt.xlim((0, N))
pyplt.xticks(ticks=hour_ticks, labels=hour_labels)
pyplt.tick_params(axis='both', labelsize=44)  
