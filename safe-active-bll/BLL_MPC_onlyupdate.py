...
u0_scaled = (80 - input_scaler_bias[0][0]) / input_scaler_scale[0][0] #70
...
# BNN prior
Uo_initial = np.array([[-0.79301643, 1.24691772, -1.03559875, 0.60808659, -0.35828972, 0.48060608], [-0.01830101, -0.03974152, -0.36886215,  0.16631031, -0.73988485, -0.34466743]], dtype=np.float32)
bo_initial = np.array([[-0.32549548], [-0.13617802]], dtype=np.float32)

theta_star = np.concatenate((Uo.flatten(), bo.flatten()), axis=0).reshape(-1, 1) # actual parameters
theta_0 = np.concatenate((Uo_initial.flatten(), bo_initial.flatten()), axis=0).reshape(-1, 1) # actual parameters
diff = theta_star - theta_0

Lambda_0 = np.load("Lambda_0_last.npy")
C = float(diff.T @ Lambda_0 @ diff)
Lambda_inv = torch.inverse(torch.from_numpy(Lambda_0))
theta_mean = torch.from_numpy(theta_0)
Q = torch.from_numpy(Lambda_0)@theta_mean
...
# ========== PRICE ==========
pyplt.figure()
pyplt.plot(time, price/10, linewidth=4)
pyplt.ylabel(r'Price [€/kWh]', fontsize=44)
pyplt.xlim((0, N))
pyplt.xlabel(r'Time [h]', fontsize=44)
...
pyplt.ylabel(r'Temperature [°C]', fontsize=44)
pyplt.xlim((0, N))
pyplt.xlabel(r'Time [h]', fontsize=44)
pyplt.xticks(ticks=hour_ticks, labels=hour_labels)
pyplt.tick_params(axis='both', labelsize=44)  
...
pyplt.figure()
pyplt.plot(time, y_RNN_den[:,0], linewidth=4, color='tab:blue')
pyplt.plot(time, y_BLL_den[:,0], linewidth=4, color='tab:orange')
pyplt.fill_between(time, y_BLL_min_beta[:,0], y_BLL_max_beta[:,0], color='orange', alpha=0.3)
pyplt.plot(time, y_min_T_variable[0:N], color='black', linewidth=4)
pyplt.plot(time, (y_max_T_value)*np.ones((N,1)), color='black', linewidth=4)
pyplt.ylabel(r'Temperature [°C]', fontsize=44)
pyplt.xlabel(r'Time [h]', fontsize=44)
pyplt.ylim((y_min_T_variable.min()-1,y_max_T_value+1))
pyplt.xlim((0, N))
pyplt.xticks(ticks=hour_ticks, labels=hour_labels)
pyplt.tick_params(axis='both', labelsize=44)  
pyplt.subplots_adjust(bottom=0.18)
...
pyplt.figure()
pyplt.plot(time, y_RNN_den[:,1]/1000, linewidth=4, color='tab:blue')
pyplt.plot(time, y_BLL_den[:,1]/1000, linewidth=4, color='tab:orange')
pyplt.fill_between(time, y_BLL_min_beta[:,1]/1000, y_BLL_max_beta[:,1]/1000, color='orange', alpha=0.3)
pyplt.plot(time, y_min_P_value[0:N]/1000, color='black', linewidth=4)
pyplt.plot(time, y_max_P_value/1000*np.ones((N,1)), color='black', linewidth=4)
pyplt.ylabel(r'Power [kW]', fontsize=44)
pyplt.xlabel(r'Time [h]', fontsize=44)
pyplt.ylim((y_min_P_value.min()/1000*0.99, y_max_P_value/1000*1.01))
pyplt.xlim((0, N))
pyplt.xticks(ticks=hour_ticks, labels=hour_labels)
pyplt.tick_params(axis='both', labelsize=44)  
pyplt.subplots_adjust(bottom=0.18)
...
pyplt.plot(time, switching_threshold*np.ones((N,1)), linestyle='--', color='red', linewidth=4)
...
pyplt.figure()
pyplt.plot(time, theta_error, linewidth=4)
pyplt.ylabel(r'$\|\theta - \theta^*\|_2$', fontsize=44)
pyplt.xlabel(r'Time [h]', fontsize=44)
pyplt.xlim((0, N))
pyplt.xticks(ticks=hour_ticks, labels=hour_labels)
pyplt.tick_params(axis='both', labelsize=44)
...
# ========== SLACK ==========
pyplt.figure()
pyplt.plot(time, max_slack_matrix, linewidth=4)
pyplt.ylabel(r'Slack', fontsize=44)
pyplt.xlim((0, N))
pyplt.xlabel(r'Time [h]', fontsize=44)
pyplt.xticks(ticks=hour_ticks, labels=hour_labels)
pyplt.tick_params(axis='both', labelsize=44)  
