...
def mpc_exploration_pessimistic(x_n, Lambda_inv_n, Q_n, theta_mean_n, theta_std_n, y_pred_n, y_std_n, U_prev, X_prev, n):
    ...
    #opti.subject_to(U==(82.94-input_scaler_bias)/input_scaler_scale)
    ...
    for h in range(H):
        ...
        # J_step -= 1000*ca.sum(Sigma_y[:,h]) #1000
        
    # Minimize
    # opti.minimize(J_step)
    opti.minimize(1000 * ca.sum(slack_sigma) + 0.1*J_step)
...
def mpc_pessimistic(x_n, Lambda_inv_n, theta_mean_n, theta_std_n, y_pred_n, y_std_n, U_prev, X_prev, switched, n):
    ...
    for h in range(H):
        ...
        # Cost function
        if not switched:
            J_step += ca.sumsqr(ca.mtimes(theta_mean_prev.T, ca.vertcat(X[:,h], 1)) - Y_ref_ca[:,h]) + ca.sum1(Sigma_y[:,h])
        else:
            J_step += ca.sumsqr(ca.mtimes(theta_mean_prev.T, ca.vertcat(X[:,h], 1)) - Y_ref_ca[:,h]) + 0.01*ca.sumsqr(U[:,h])
        # if h > 0 and switched:
            #J_step += 10 * ca.sumsqr(U[:,h] - U[:,h-1])
...
for n in range(N):
    ...
    # if not switched and n > 5 and all((J_pess[n - i] - J_opt[n - i] <= threshold) for i in range(1, 6)):  
