...
Y_ref_den = 83.85
Y_ref = (Y_ref_den - output_scaler_bias) / output_scaler_scale
delta = 0.0001
threshold = 0.01
...
def mpc_exploration_pessimistic(x_n, Lambda_inv_n, Q_n, theta_mean_n, theta_std_n, y_pred_n, y_std_n, U_prev, X_prev):
    ...
    # Theta_mean = opti.variable(n_theta, H)
    Theta_var = opti.variable(n_theta, H)
    # slack_theta = opti.variable(n_theta, H)
    
    # Needed variables
    Lambda_inv_prev = ca.MX(Lambda_inv_n.detach().cpu().numpy())  
    Q_prev = ca.MX(Q_n.detach().cpu().numpy())
    ...
    for h in range(H):
        ...
        opti.subject_to(Theta_var[:,h] == sigma2*ca.diag(Lambda_inv_prev))
        # opti.subject_to(Theta_var[:,h] >= epsilon_theta - slack_theta[:,h])
        # Theta_var = sigma2*ca.diag(Lambda_inv_prev)
        # opti.subject_to(Theta_var >= epsilon_theta - slack_theta[:,h])

        # Pessimistic set constraint
        if h > 0:
            # opti.subject_to(lower_bound[:,h] >= lower_bound[:,h-1])
            # opti.subject_to(lower_bound[:,h] >= ca.mtimes(theta_mean_prev.T, ca.vertcat(X[:,h], 1)) - ca.sqrt(beta_n) * ca.sqrt(sigma2 * (1 + ca.mtimes([ca.vertcat(X[:,h], 1).T, Lambda_inv_prev, ca.vertcat(X[:,h], 1)]))))
            # opti.subject_to(upper_bound[:,h] <= upper_bound[:,h-1])
            # opti.subject_to(upper_bound[:,h] <= ca.mtimes(theta_mean_prev.T, ca.vertcat(X[:,h], 1)) + ca.sqrt(beta_n) * ca.sqrt(sigma2 * (1 + ca.mtimes([ca.vertcat(X[:,h], 1).T, Lambda_inv_prev, ca.vertcat(X[:,h], 1)]))))
        ...
        J_step += 0.01*ca.sumsqr(U[:,h])
        J_step -= 10*ca.sum(Theta_var[:,h])

        # Recursive update
        denom = 1 + ca.mtimes([ca.vertcat(X[:,h], 1).T, Lambda_inv_prev, ca.vertcat(X[:,h], 1)])
        Lambda_inv_new = Lambda_inv_prev - ca.mtimes([ca.mtimes(Lambda_inv_prev, ca.vertcat(X[:,h], 1)), (ca.mtimes(Lambda_inv_prev, ca.vertcat(X[:,h], 1))).T]) / denom
        Lambda_inv_prev = Lambda_inv_new
        # Q_new = Q_prev + ca.mtimes(ca.vertcat(X[:,h], 1), ca.mtimes(theta_mean_prev.T, ca.vertcat(X[:,h], 1)).T)
        # if h < H - 1:
        #     opti.subject_to(Theta_mean[:, h+1] == ca.mtimes(Lambda_inv_new, Q_new))
        # Q_prev = Q_new

    # Minimize
    opti.minimize(J_step)
    #opti.minimize(100 * ca.sumsqr(slack_theta) + J_step)
    # opti.minimize(100000*ca.sumsqr(slack_sigma) + 0.00001*J_step)
    #opti.subject_to(J == -1*ca.sumsqr(Sigma_y) + 0.1*J_step)
    # opti.subject_to(J == 10*ca.sumsqr(slack_sigma) + 0.1*J_step)
    # opti.minimize(J)
    #opti.minimize(ca.sum(slack_sigma) + ca.sumsqr(ca.mtimes(Theta[:,h].T, ca.vertcat(X[:,h], 1)) - Y_ref))    
    #opti.minimize(ca.sumsqr(slack_sigma) + 0.001*ca.sumsqr(U-(75-input_scaler_bias)/input_scaler_scale))
    #opti.minimize(-1*ca.sumsqr(Sigma_y))
    #opti.minimize(ca.sumsqr(slack_theta) + 0.001*ca.sumsqr(U-(75-input_scaler_bias)/input_scaler_scale))
    #opti.minimize(-1*ca.sumsqr(Theta_var))
    ...
    # opti.set_initial(slack_theta, 0)
    opti.set_initial(Theta_var, np.tile(np.array(theta_std_n**2).reshape(-1, 1), (1, H)))
    # opti.set_initial(Sigma_y, np.tile(np.array(y_std_n).reshape(-1, 1), (1, H)))
    # opti.set_initial(slack_sigma, 0)
    #opti.set_initial(Theta_mean, np.tile(np.array(theta_mean_n).reshape(-1, 1), (1, H)))
    ...
    # slack_max = np.max(np.max(sol.value(slack_theta)))
    # slack_first = np.max(sol.value(slack_theta[:,0]))
    slack_max = 0
    slack_first = 0
...
# Pessimistic MPC
def mpc_pessimistic(x_n, Lambda_inv_n, Q_n, theta_mean_n, theta_std_n, y_pred_n, y_std_n, U_prev, X_prev, switched):
    ...
    for h in range(H):
        ...
        # Cost function
        J_step +=  ca.sumsqr(ca.mtimes(theta_mean_prev.T, ca.vertcat(X[:,h], 1)) - Y_ref_ca[:,h])
        if switched:
            J_step += 0.01*ca.sumsqr(U[:,h])
        # Recursive update
        denom = 1 + ca.mtimes([ca.vertcat(X[:,h], 1).T, Lambda_inv_prev, ca.vertcat(X[:,h], 1)])
...
# Optimistic MPC
def mpc_optimistic(x_n, Lambda_inv_n, Q_n, theta_mean_n, theta_std_n, y_pred_n, y_std_n, U_prev, X_prev):
    ...
    for h in range(H):
        ...
        # Constraint on Theta
        # opti.subject_to(Theta[:,h] <= theta_mean_prev + ca.sqrt(beta_n) * theta_std_prev)
        # opti.subject_to(Theta[:,h] >= theta_mean_prev - ca.sqrt(beta_n) * theta_std_prev)
        ...
        Lambda_inv_new = Lambda_inv_prev - ca.mtimes([ca.mtimes(Lambda_inv_prev, ca.vertcat(X[:,h], 1)), (ca.mtimes(Lambda_inv_prev, ca.vertcat(X[:,h], 1))).T]) / denom
        Lambda_inv_prev = Lambda_inv_new
        # Q_new = Q_prev + ca.mtimes(ca.vertcat(X[:,h], 1), ca.mtimes(Theta[:,h].T, ca.vertcat(X[:,h], 1)).T)
        # if h < H - 1:
        #     opti.subject_to(Theta_mean[:, h+1] == ca.mtimes(Lambda_inv_new, Q_new))
        # Q_prev = Q_new
...
# Uo_p = np.array(Uo).flatten()  
# bo_p = float(bo)              
# theta_vec = np.append(Uo_p, bo_p) 
# theta_mean = torch.tensor(theta_vec, dtype=torch.float32).reshape(-1, 1)
...
for n in range(N):
    # Exploration
    U_mpc_exploration, slack_max[n,:], slack_first[n,:], U_prev, X_prev = mpc_exploration_pessimistic(x0, Lambda_inv, Q, theta_mean, theta_std, y_pred, y_std, U_prev, X_prev)
    U_explo.append(U_mpc_exploration)
    # Pessimistic
    U_mpc_pess, J_pess[n,:], U_prev_pess, X_prev_pess = mpc_pessimistic(x0, Lambda_inv, Q, theta_mean, theta_std, y_pred, y_std, U_prev_pess, X_prev_pess, switched)
    U_pess.append(U_mpc_pess)
    # Optimistic
    J_opt[n,:], U_prev_opt, X_prev_opt = mpc_optimistic(x0, Lambda_inv, Q, theta_mean, theta_std, y_pred, y_std, U_prev_opt, X_prev_opt)
    ...
    if not switched and n >=100:
        switched = True
    
    if n == 0:
        y0 = np.array(U_mpc_exploration-0.08).reshape(1,1)
...
# pyplt.plot(time, slack_max, label="Slack max")
# pyplt.plot(time, slack_first, label="Slack first")
# pyplt.legend(fontsize=10)
# pyplt.ylabel('Slack', fontsize=10)
# pyplt.xlim((0, N))
# pyplt.show()
